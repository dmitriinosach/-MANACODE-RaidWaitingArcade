...
local function ensureUI(canvas)
    ...
    ui.nums = CreateFrame("Frame", nil, canvas)
    ui.nums:SetFrameLevel(canvas:GetFrameLevel() + 3)
    ui.nums:SetAllPoints(canvas)
    ui.nums:Hide()
    ui.board = CreateFrame("Frame", nil, canvas)
    ...
    for i = 1, N * N do
        ...
        t:SetTexture(C_WELL[1], C_WELL[2], C_WELL[3], C_WELL[4])
        ui.well[i] = t
    end
    for i = 1, TOP do
    ...
    ui.ruleCard:SetTexture(C_RULE[1], C_RULE[2], C_RULE[3], C_RULE[4])
...
function Game:Dress(f, rung)
    ...
    f.center:SetFont(font, floor(self.cs * k), "THICKOUTLINE")
...
function Game:Layout()
    ...
    u.name:SetText(own and ns.T("merge.rungLabel", at, MAXRUNG)
                   or (s and (GetItemInfo(s.item) or "")) or ns.T("merge.crown"))
