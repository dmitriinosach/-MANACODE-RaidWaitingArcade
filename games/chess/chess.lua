...
local function ensureView(canvas)
    ...
    board:Hide()
    local deck = CreateFrame("Frame", nil, board)
    ...
    marks = CreateFrame("Frame", nil, canvas)
    ...
    ui.plate:SetTexture(C_NICHE[1], C_NICHE[2], C_NICHE[3], C_NICHE[4])
    local function edge(p1, p2, w, h)
        local t = ui.panel:CreateTexture(nil, "BORDER")
        t:SetTexture(C_EDGE[1], C_EDGE[2], C_EDGE[3], C_EDGE[4])
    ...
    for k = 1, LOG_VIS do
        ...
        r.lit:SetTexture(0.16, 0.14, 0.09, 1)
    ...
    ui.pickBox = CreateFrame("Frame", nil, canvas)
    ...
    ui.glow = CreateFrame("Frame", nil, canvas)
...
function Game:Build()
    ...
    for k = 1, 64 do
        ...
        sq:SetTexture(col[1], col[2], col[3])
...
function Game:DrawMarks()
    ...
    for k = 1, 64 do
        ...
        local m = board.mark[k]
        if col then
            m:SetTexture(col[1], col[2], col[3], col[4])
            m:Show()
        else
            m:Hide()
        end
    ...
    if self.ban and self.sel and not self.promo and not self.over then
        for i in pairs(self.ban) do
            if n < MAX_DOTS then
                ...
                d:SetTexture(C_BAN[1], C_BAN[2], C_BAN[3], C_BAN[4])
        ...
        for i = 1, 64 do
            if self.can[i] and n < MAX_DOTS then
                ...
                if self.p[i] or i == eating then
                    d:SetTexture(C_TAKE[1], C_TAKE[2], C_TAKE[3], C_TAKE[4])
                    d:SetWidth(self.cell)
                    d:SetHeight(self.cell)
                    d:SetPoint("BOTTOMLEFT", self.canvas, "BOTTOMLEFT", x, y)
                else
