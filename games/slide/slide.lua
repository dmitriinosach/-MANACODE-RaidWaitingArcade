...
function Game:ShowPicture()
    ...
    if self.pic then
        ...
        ns.Paint(view.mini.icon, BLANK[1], BLANK[2], BLANK[3])
...
local function makeTile(parent)
    ...
    ns.Paint(c.tag, C_TAG[1], C_TAG[2], C_TAG[3], C_TAG[4])
    c.tag:Hide()
...
local function ensureView(canvas)
    ...
    view = {}
    view.deco = ns.NewFrame("Frame", nil, canvas)
    ...
    view.field = ns.NewFrame("Frame", nil, canvas)
    ...
    ns.Paint(view.well, C_BED[1], C_BED[2], C_BED[3], 1)
    view.deck = ns.NewFrame("Frame", nil, view.field)
    ...
    ns.Paint(view.bed, C_BED[1], C_BED[2], C_BED[3], 1)
    view.pit = ns.NewFrame("Frame", nil, view.deck)
    ...
    view.pit:Hide()
    view.hl = ns.NewFrame("Frame", nil, canvas)
    ...
    for k = 1, 4 do
        ...
        ns.Paint(view.hl.dark[k], 0, 0, 0, EDGE_DARK_A)
        view.hl.light[k] = view.hl:CreateTexture(nil, "OVERLAY")
        ns.Paint(view.hl.light[k], 1, 1, 1, EDGE_A)
    end
    view.hl:Hide()
    view.shot = ns.NewFrame("Frame", nil, canvas)
    ...
    view.shot:Hide()
    view.panel = ns.NewFrame("Frame", nil, canvas)
    ...
    ns.Paint(view.panelEdge, C_FRAME[1] * 0.55, C_FRAME[2] * 0.55, C_FRAME[3] * 0.55, 1)
    ...
    ns.Paint(view.plate, C_PLATE[1], C_PLATE[2], C_PLATE[3], 1)
    view.col = ns.NewFrame("Frame", nil, view.panel)
    ...
    local function rule(y)
        ...
        ns.Paint(t, C_RULE[1], C_RULE[2], C_RULE[3], 1)
        t:SetHeight(1)
    ...
    view.note = ns.NewFrame("Frame", nil, canvas)
    ...
    ns.Paint(view.notePlate, 0, 0, 0, 0.7)
    view.note:Hide()
    ...
    ns.Paint(ruleAxis, C_RULE[1], C_RULE[2], C_RULE[3], 1)
...
function Game:Tag(c, k)
    ...
    if not ns.SetFont(c.tagText, TAG_FONT, px, "") then
        ns.SetFont(c.tagText, (c.tagText:GetFont()), px, "")
    end
    c.tagText:SetText(k)
...
function Game:Face(c, k)
    ...
    if self.pic then
        ...
        ns.Paint(c.icon, BLANK[1], BLANK[2], BLANK[3])
...
local function ensurePicker(canvas)
    ...
    picker = ns.NewFrame("Frame", nil, canvas)
    ...
    ns.Paint(picker.dim, 0, 0, 0, COVER_A)
