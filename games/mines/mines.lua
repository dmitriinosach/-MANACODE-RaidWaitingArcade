...
local function ensureView(canvas)
    ...
    field:Hide()
    deck = ns.NewFrame("Frame", nil, field)
    ...
    ns.Paint(ui.ruleAxes, C_RULE[1], C_RULE[2], C_RULE[3], C_RULE[4])
...
local function border(r, g, b)
    ...
    ui.press = ns.NewFrame("Frame", nil, canvas)
    ...
    ui.flash = ns.NewFrame("Frame", nil, canvas)
    ...
    ui.flash:Hide()
    ui.blast = ns.NewFrame("Frame", nil, canvas)
...
function Game:Start(seed, moves)
    ...
    if names[pick] then
        ...
        if self.bombID and not ns.Compat.ItemIcon(self.bombID) then ns.Compat.ItemInfo(self.bombID) end
    end
    self.mine, self.near = {}, {}
    self.open, self.flag = {}, {}
    for i = 1, self.n do
...
function Game:Build()
    ...
    for i = 1, self.n do
        ...
        if not ns.SetFont(t.num, NUM_FONT, px, "THICKOUTLINE") then
            ns.SetFont(t.num, (t.num:GetFont()), px, "THICKOUTLINE")
        end
    end
...
local function tone(t, tn)
    ...
    ns.Paint(t.face, f[1], f[2], f[3], f[4])
    ns.Paint(t.rim, r[1], r[2], r[3], r[4])
end
function Game:Draw()
