...
local function makeIcon(h)
    local b = ns.NewFrame("Button", nil, h)
...
local function buildHead(parent)
    local h = ns.NewFrame("Frame", nil, parent)
    h:SetHeight(HEAD_H)
    ...
    h.musicNext:Hide()
    h.sound = ns.NewFrame("Frame", nil, h)
    ...
    h.mate = ns.NewFrame("Frame", nil, h)
...
local function buildBar(parent)
    local b = ns.NewFrame("Frame", nil, parent)
    ...
    b.clockBox = ns.NewFrame("Frame", nil, b)
...
local function buildGrip()
    local g = ns.NewFrame("Button", nil, frame)
    g:SetSize(22, 22)
    ...
    hi:SetAlpha(0.35)
    local badge = ns.NewFrame("Frame", nil, UIParent)
...
local function build()
    frame = ns.NewFrame("Frame", "RaidWaitingArcadeFrame", UIParent)
    frame:SetSize(W, H)
    ...
    frame.close = ns.NewFrame("Button", nil, frame, "UIPanelCloseButton")
    frame.grip = buildGrip()
    frame.miniStrip = ns.NewFrame("Frame", nil, frame)
    ...
    canvas = ns.NewFrame("Frame", "RaidWaitingArcadeCanvas", frame)
...
local function rebuildBar()
    ...
    sepPool = sepPool or ns.NewPool(function()
        local s = ns.NewFrame("Frame", nil, bar)
    ...
    for _, it in ipairs(items) do
        ...
        if it.gap and ((it.side == "right" and prevR) or (it.side ~= "right" and prevL)) then
            ...
            ns.Paint(sep.line, rc[1], rc[2], rc[3], rc[4])
            sep:ClearAllPoints()
        end
        f:ClearAllPoints()
...
local function showFinale()
    if not finaleUI then
        finaleUI = ns.NewFrame("Frame", nil, canvas)
        ...
        local plate = ns.NewFrame("Frame", nil, finaleUI)
...
local function syncHeld()
    ...
    if not heldUI then
        heldUI = ns.NewFrame("Frame", nil, canvas)
