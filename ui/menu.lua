...
local function glowTile(t, on)
    if on then
        ...
        ns.Gradient(t.sheen, "HORIZONTAL", g[1], g[2], g[3], SHEEN_A, g[1], g[2], g[3], 0)
...
local function makeTile(parent)
    local t = ns.NewFrame("Button", nil, parent)
    ...
    t.halo:Hide()
    t.cell.art = ns.NewFrame("Frame", nil, t.cell)
...
local function buildLinks(parent)
    ...
    p.probe:Hide()
    p.addr = ns.NewFrame("EditBox", nil, p.box)
...
local function build()
    ...
    frame = ns.NewFrame("Frame", nil, parent)
    ...
    probe:Hide()
    games.pager = ns.NewFrame("Frame", nil, games)
